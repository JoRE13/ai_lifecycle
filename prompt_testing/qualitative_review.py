...
META_FILE = ROOT / "test_cases.csv"
PROMPT_VERSIONS = ['old', 'lite_med', 'flash_low', 'flash_med', 'flash_dev_v5', 'flash_med_v5', 'flash_med_v6']
...
def normalize_image_key(value: Any) -> str:
    ...
    # Match derived files like "1.txt" back to source images like "1.png".
...
def is_verdict_correct(expected_verdict: str, predicted_verdict: str) -> bool:
    return norm_token(expected_verdict) == norm_token(predicted_verdict)
...
def build_scored_rows() -> pd.DataFrame:
    ...
    meta["image"] = meta["image"].map(normalize_image_key)

    rows: list[dict[str, Any]] = []
    for version in PROMPT_VERSIONS:
        result_path = ROOT / f"results_{version}.csv"
        result_df = pd.read_csv(result_path)
        if "latency" not in result_df.columns:
            result_df["latency"] = pd.NA
        result_df["image"] = result_df["file_path"].map(normalize_image_key)

        merged = result_df.merge(meta, on="image", how="left", suffixes=("_result", "_meta"))
        matched_cases = merged["id"].notna().sum()
        total_cases = len(merged)
        print(f"{version}: matched {matched_cases}/{total_cases} rows to test_cases.csv")
        for r in merged.itertuples(index=False):
            message = norm_text(r.message_is)
            response_type = norm_text(r.response_type)
            verdict = norm_text(r.verdict)
            expected_verdict = norm_text(r.expected_verdict_meta)
            ...
            verdict_correct = is_verdict_correct(expected_verdict, verdict)
...
def save_outputs(scored: pd.DataFrame) -> None:
    ...
    latency_numeric = pd.to_numeric(scored["latency"], errors="coerce")

    summary = (
        scored.assign(hint_usefulness_numeric=hint_numeric, latency_numeric=latency_numeric)
        .groupby("prompt_version")
        .agg(
            total_cases=("id", "count"),
            correct_verdicts=("correct", "sum"),
            ...
            avg_latency=("latency_numeric", "mean"),
        )
        .reset_index()
    )
    ...
    mode_summary["correct_rate"] = (mode_summary["correct_rate"] * 100).round(1)
    ...
    mode_summary["avg_latency"] = mode_summary["avg_latency"].round(2)
